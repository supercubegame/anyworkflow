...
### 还欠一条：送达证据
回写与 attest 是刚接上的。第一轮（PR #14）三个 job 全绿、attest 也绿，**但那一轮只走了 commit 评论这一条通道**，而且 attest 至今没在 live CI 里红过一次。

从没红过的断言，和空断言在报告上长得一样。所以 `VERIFY-EVIDENCE-0003` 还不能写 —— 它要等两件事：**PR 评论那条通道也真的走一遍**，以及**attest 真的因为送达坏掉而红一次**。
