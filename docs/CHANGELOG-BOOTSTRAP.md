...
这一步的目标不是“备份完整”，是先保证**这份备份仓自己不会安静地说谁**：
- 规则文件必须同步
- 恢复文档必须存在
- 盲区清单必须存在
...
**这一步没有做的一半，明写在这里：**
1. 真拄本（`agents/*.md`、`vendor/ci-workflows/report.yml`）仍然在闸门视野之外,可以被清成空壳而全绿。修法是“备份类需求”那四条：清单双向、正文锚点、恢复充分性、新鲜度。
...
**当前证据强度**：待观察。接线是真的、离线自证是真的（九个合成样本 + 九个变异体全部判红），但“它在 live CI 里真的把评论送出去了”这句话**在观察到那次运行之前不算数**,那条记录留给 `VERIFY-EVIDENCE-0003`，写实测值，不写预期。
