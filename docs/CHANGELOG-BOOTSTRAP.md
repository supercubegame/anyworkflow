...
**順带抓到一件不在计划里的事：写入通道会悄悄改写字。** 逐个文件读回核对 blob 哈希时发现 `说谎`→`说谁`、`抄本`→`拄本`、`骨架`→`骶架`、`分岔`→`分岘`,全是形近字，**每一处字节长度都没变**，于是两份文档读回来体积和本地完全一致，只有哈希不同。这条已经写进 `MINIMAL-GATE.md` 的“不守 6”，并且把真抄本那四条断言里的 blob 哈希提成了最高优先级。
