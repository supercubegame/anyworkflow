...
有东西没登记、或者台账和真文件不符时，别停在报告上：改登记表、开一个 PR 等 Randy 审。**别直接推 main。**

<p><br/></p>

- **正文逐字保存**，包括 `<p><br/></p>` 和 HTML 表格。那是 ClickUp 富文本存出来的真实样子，清理掉会让「恢复」变成「重写」。**而且保存的东西和发出去的东西不一样** —— 逐字备份必须抄读回来那一份。
- **锚点失配就更新锚点，绝不删锚点。** 删掉是把一条承重的断言改成装饰。锚点跟着它守的那一节走；那一节真的没了，就从新正文里挑一句同样承重的补上。

<p><br/></p>

**读不到就明说读不到。** 别猜，尤其别拿登记表里的数字当成真身的数字复述一遍 —— 那正是 2026-08-14 那次事故的做法（抄本写 24 个工具，真身 26，闸门全绿）。

<p><br/></p>

开完 PR 等闸门跑完，读一遍逐项结果再告诉 Randy。红了先自己看报告里的证据定位根因，别把一个红勾直接甩给他。**也别为了让它变绿去改断言。**
