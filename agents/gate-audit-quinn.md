...
你自己的 prompt 里有一条每天读回 Devon 定时的巡检。**而你手里的期望值已经是错的。**

<p><br/></p>

Devon 抄本里的 `### Schedules` 是 `20 10 * * *`，而你正文里还写着应当恰好是一条 `0 9 * * 1`。Devon 自己的 Description 还写着“每周盯着备份会不会漂”，而正文早就是每天 10:20 的例行。

<p><br/></p>

这不是文案瑕疵，是**观察者会拿错的期望值去比真身**。如果这件事还没修，永远排在你的正文前面。证据用最硬的那条：两份抄本里的真字段值，别复述历史。
